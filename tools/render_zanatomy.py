#!/usr/bin/env python3
"""Render Atlas's photoreal muscle-map pack from the Z-Anatomy model.
...
WHAT IT PRODUCES (into $ATLAS_ANATOMY_OUT, default /tmp/atlas_pack)
  anatomy_body_front.png / anatomy_body_back.png          — colour écorché
  anatomy_body_front_gray.png / anatomy_body_back_gray.png — grey tint source
  anatomy_<AnatomicalMuscle rawValue>.png                  — one alpha mask
                                                            per muscle head
  Every image shares one locked orthographic camera per facing, so the
  masks are pixel-aligned to the bodies by construction.
...
NOTES THAT MATTER (learned the hard way)
  ...
  * The colour bodies merge every belly into ONE decimated mesh; Cycles'
    geometry export OOMs on 500+ separate high-poly objects.
"""
...
# Non-muscle structures to drop from the muscle bellies / masks.
...
def isolate_muscular(vl):
    ...
    the Muscular system branch (avoids evaluating ~4500 objects -> OOM)."""
    parent = {}
...
def flesh_material(name, base, subsurface):
    ...
    if subsurface:
        setin(b, "Subsurface Radius", (0.03, 0.009, 0.006))
        setin(b, "Subsurface Scale", 0.02)
    return m
...
def main():
    ...
    log("muscle meshes:", len(allm), "bellies:", len(bellies))
    ...
    # locked cameras + studio rig, framed on the bellies
    ...
    height = max(0.1, maxs[2] - mins[2]); dist = height * 3 + 1
    ...
    def cam(name, y, flip):
        ...
        return c
    cams = {"front": cam("CF", mins[1] - dist, False),
            "back": cam("CB", maxs[1] + dist, True)}

    def sun(name, frm, e, col):
        ld = bpy.data.lights.new(name, "SUN"); ld.energy = e; ld.color = col; ld.angle = 0.15
    ...
    sun("kF", (center[0] - height, mins[1] - dist, center[2] + height * 0.6), 4.0, (1, 0.96, 0.92))
    sun("fF", (center[0] + height, mins[1] - dist, center[2]), 1.2, (0.9, 0.95, 1))
    sun("kB", (center[0] - height, maxs[1] + dist, center[2] + height * 0.6), 4.0, (1, 0.96, 0.92))
    sun("fB", (center[0] + height, maxs[1] + dist, center[2]), 1.2, (0.9, 0.95, 1))
    ...
    def render(cam_key, path, transform, samples, denoise):
        ...
        sc.view_settings.exposure = 0.4 if transform == "AgX" else 0.0
    ...
    # ---- beauty bodies: merge every belly into one decimated mesh ----
    def beauty(material, suffix):
        ...
        merged.data.materials.append(material); merged.hide_render = False
        merged.modifiers.new("d", "DECIMATE").ratio = 0.13
        render("front", f"{OUT}/anatomy_body_front{suffix}.png", "AgX", 32, True)
        render("back", f"{OUT}/anatomy_body_back{suffix}.png", "AgX", 32, True)
        bpy.data.objects.remove(merged, do_unlink=True)

    beauty(flesh_material("muscle", (0.45, 0.08, 0.07), 0.22), "")
    beauty(flesh_material("muscle_gray", (0.66, 0.66, 0.66), 0.0), "_gray")
